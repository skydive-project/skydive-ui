var config = {
    ...
    "nodeMenu": function (node) {
        ...
        if (node.data.Expandable && node.data.Expandable === "True") {
            menu.push ({ class: "", text: "Expend", disabled: false, callback: () => { window.location.replace("?data=/assets/"+node.data.Name+".json"); } })
            menu.push ({ class: "", text: "Home", disabled: false, callback: () => { window.location.replace("?data=/assets/dump.json"); } }) 
        }
